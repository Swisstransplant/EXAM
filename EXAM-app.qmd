...
|                        | $y_{\text{perf. start}}$ | perfusate temperature at start                          | $\textdegree$C |
...
| Quality control indicators | $D^2_{temp}$     | Distance of the case from other cases with respect to temperature (a lower score is better). It can be seen as the deviation from the norm. |  |
|                        | $P_{temp}$     | Percentile rank of the $D^2_{temp}$ indicators. For example, a score of 0.90 means that 90% of the cases had a better result (thus, a lower score is better).  |  |
...
output$Tab4 <- renderTable({
  ...
  colnames(tab) = c("<i>m</i><sub>ice</sub>", "<i>SD</i><sub>ice</sub>", 
                    "<i>&#916t</i><sub>y>2.5</sub>", "<i>y</i><sub>perf. start</sub>",
